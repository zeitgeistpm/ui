...
import React, { FC, Fragment, PropsWithChildren, useRef, useState, useEffect } from "react";
import {
  ArrowRight,
  BarChart,
  Bell,
...
const BalanceRow = ({
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg bg-white/10 px-2.5 py-2 backdrop-blur-sm transition-all hover:bg-white/15 md:gap-3 md:px-3 md:py-2.5">
      <img src={imgPath} className="h-6 w-6 shrink-0 rounded-full ring-2 ring-white/10 md:h-7 md:w-7" />
      <div
...
> = ({ onClick, disabled, children }) => {
  return (
    <button
      className={`flex w-[185px] cursor-pointer items-center justify-center rounded-full border-2 border-white/20 bg-white/10 px-6 font-semibold leading-[40px] text-white shadow-md backdrop-blur-lg transition-all hover:bg-white/15 hover:border-white/30 hover:shadow-lg hover:shadow-ztg-green-500/10 disabled:cursor-default disabled:opacity-30 disabled:hover:bg-white/10 disabled:hover:border-white/20 disabled:hover:shadow-md`}
      onClick={onClick}
      disabled={disabled}
    >
      {children}
      ...
      {!connected ? (
        ...
        >
          ...
          {hovering === true && locationAllowed !== true ? (
            <div className="absolute bottom-0 right-0 rounded-lg border-2 border-ztg-red-500/40 bg-ztg-red-500/20 px-3 py-2 text-sm font-semibold text-white/90 backdrop-blur-sm shadow-md">
            ...
            {({ open, close }) => {
              ...
              <>
                <div className="w-full max-w-[200px] md:max-w-[200px]">
                  <div className="relative flex h-9 w-full md:h-11 md:w-full md:shrink-0 overflow-hidden">
                    <Menu.Button className="w-full md:w-full md:shrink-0">
                      <div className="relative z-[110] flex h-full w-full min-w-0 cursor-pointer items-center rounded-full md:w-full md:shrink-0">
                        <div
                          className={`flex h-full w-full min-w-0 items-center justify-center gap-2 rounded-full border-2 bg-white/10 py-1.5 px-2 text-white shadow-md backdrop-blur-lg transition-all hover:bg-white/15 hover:shadow-lg hover:border-white/30 md:gap-2.5 md:py-2 md:px-3 md:min-w-0 md:shrink-0 ${
                            open
                        ...
                        >
                          ...
                          <span
                            className={`flex-1 min-w-0 flex items-center justify-center overflow-hidden text-center text-xs font-semibold transition-all md:min-w-0 md:text-center md:text-sm md:whitespace-nowrap ${
                          ...
                          <div className="shrink-0">
                            <ChevronDown
                              size={16}
                              viewBox="4 3 16 16"
                              className={`transition-all duration-200 md:h-4 md:w-4 ${
                                open
                ...
                >
                  <Menu.Items
                    className="fixed left-0 right-0 z-[120] w-screen origin-top-right border-t-2 border-white/10 px-4 py-4 shadow-2xl backdrop-blur-lg focus:outline-none md:fixed md:inset-auto md:left-auto md:h-auto md:w-80 md:max-h-[calc(100vh-100px)] md:overflow-visible md:rounded-lg md:border-2 md:border-white/10 md:bg-ztg-primary-700/95 md:backdrop-blur-lg md:ring-2 md:ring-white/5 md:shadow-2xl md:mt-2 md:px-5 md:py-3"
                    style={dropdownPosition ? {
                      right: isMobile ? 0 : `${dropdownPosition.right}px`,
                  ...
                  >
                    <div className="flex h-full flex-col">
                      ...
                      {isMobile && (
                        <div className="mb-3 flex shrink-0 items-center justify-between border-b-2 border-white/10 pb-2 px-4">
                    ...
                    <div className="flex-1 overflow-y-auto pb-4">
                      <div className={`mb-3 flex flex-col gap-2 border-b-2 border-white/10 px-4 py-3 md:gap-3 md:px-6 md:py-4`}>
                        <BalanceRow
                        ...
                        <Menu.Item>
                          {({ active }) => (
                            <Link
                              ...
                              className="mt-1"
                            >
                              <div className={`group flex cursor-pointer items-center justify-between rounded-lg px-2.5 py-2 transition-all md:px-3 md:py-2.5 ${
                                active ? "bg-white/20 shadow-sm" : "bg-white/10 hover:bg-white/20"
                              }`}>
                                ...
                                <ArrowRight
                                  size={14}
                                  className="text-ztg-green-400 transition-transform group-hover:translate-x-1 md:h-4 md:w-4"
                                />
                              </div>
                            </Link>
                          )}
                      ...
                      <div className={`flex flex-col gap-1 px-4 py-2 md:gap-1.5 md:px-6`}>
                        {/* Notifications Section */}
                        <Menu.Item>
                          {({ active }) => (
                            <div
                              className={`group flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-2 text-white/90 transition-all md:gap-3 md:px-3 md:py-2.5 ${
                                active ? "bg-white/20 shadow-sm" : "hover:bg-white/20"
                              }`}
                              onClick={() => setShowNotifications(!showNotifications)}
                            >
                              <div className="relative">
                                <Bell className="text-ztg-green-400 transition-colors" size={16} />
                                {hasNotifications && (
                                  <div className="absolute -right-0.5 -top-0.5 h-1.5 w-1.5 rounded-full bg-ztg-green-500 md:-right-1 md:-top-1 md:h-2 md:w-2"></div>
                                )}
                              </div>
                              ...
                              {hasNotifications && (
                                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-ztg-green-500/20 text-[10px] font-semibold text-ztg-green-400 md:h-8 md:w-8 md:text-xs">
                                  {alerts.length}
                                </span>
                              )}
                            </div>
                        ...
                        {showNotifications && hasNotifications && (
                          <div className="mb-2 max-h-[250px] space-y-1.5 overflow-y-auto rounded-lg bg-white/5 p-2 md:max-h-[300px] md:space-y-2 md:p-3">
                            {alerts.map((alert) => (
                              <NotificationItem
                                key={String(alert.id)}
                                alert={alert}
                        ...
                        {isNovaWallet !== true && (
                          <Menu.Item>
                            {({ active }) => (
                              ...
                              >
                                <User className="text-ztg-green-400 transition-colors" size={16} />
                        ...
                        <Menu.Item>
                          {({ active }) => (
                            <Link href={`/portfolio/${realAddress}`}>
                              <div className={`group flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-2 text-white/90 transition-all md:gap-3 md:px-3 md:py-2.5 ${
                                active ? "bg-white/20 shadow-sm" : "hover:bg-white/20"
                              }`}>
                                <BarChart className="text-ztg-green-400 transition-colors" size={16} />
                                <span className="text-xs font-medium md:text-sm">
                                  Portfolio
                                </span>
                              </div>
                        ...
                        <Menu.Item>
                          {({ active }) => (
                              ...
                              >
                                <Settings className="text-ztg-green-400 transition-colors" size={16} />
                                <span className="text-xs font-medium md:text-sm">
                                  Settings
                                </span>
                              </div>
                        ...
                        <Menu.Item>
                          {({ active }) => (
                            <div
                              className={`group flex cursor-pointer items-center gap-2 rounded-lg border-2 border-ztg-red-500/40 bg-ztg-red-900/30 px-2.5 py-2 text-white/90 backdrop-blur-sm transition-all hover:border-ztg-red-500/60 hover:bg-ztg-red-900/50 md:gap-3 md:px-3 md:py-2.5 ${
                            ...
                            >
                              <LogOut className="text-ztg-red-400 transition-colors" size={16} />
                              <span className="text-xs font-medium md:text-sm">
                                Disconnect
                              </span>
                            </div>
...
const NotificationItem = ({
  ...
  return (
    <div
      className="cursor-pointer rounded-lg bg-white/10 px-3 py-2 transition-all hover:bg-white/20"
      onClick={handleClick}
    >
      <div className="text-xs font-semibold text-white/90">{getLabel()}</div>
      {getDescription() && (
