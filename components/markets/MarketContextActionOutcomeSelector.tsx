...
export type MarketContextActionOutcomeSelectorWithCombinatorialProps = {
  market: FullMarketFragment;
        ...
        <div className="center gap-3">
          <Listbox.Button onClick={() => setOpen(!open)}>
            <div className="center gap-2 text-2xl md:text-xl lg:text-2xl">
              <TruncatedText
                length={24}
                text={'CategoricalOutcome' in selected
...
export const MarketContextActionOutcomeSelectorWithCombinatorial = ({
  market,
  selected,
  options,
  disabled,
  hideValue,
  onChange,
}: MarketContextActionOutcomeSelectorWithCombinatorialProps) => {
  const [open, setOpen] = useState(false);
  const [search, setSearch] = useState<string | undefined>();
  const inputRef = useRef<HTMLInputElement>(null);
