...
export const MenuItem: FC<PropsWithChildren<MenuItemProps>> = ({
  ...
  return (
    <WrapComponent href={href}>
      <button
        className={`border-b-2lack group flex w-[68px] rounded-full border-2 bg-black p-5 text-white/90 transition-all duration-300 hover:border-sunglow-2 hover:bg-black hover:text-sunglow-2 focus:border-ztg-blue focus:bg-ztg-blue focus:text-white/90 ${
          active && "border-ztg-blue bg-ztg-blue"
        }`}
      >
        <div className="center relative">
          <IconComponent size={24} />
          <div className="invisible absolute left-14 whitespace-nowrap rounded bg-sunglow-2 px-2.5 py-1 text-lg text-black opacity-0 transition-all duration-300 group-hover:visible group-hover:opacity-100 group-focus:bg-ztg-blue group-focus:text-white/90">
            {textLabel}
          </div>
        </div>
      </button>
