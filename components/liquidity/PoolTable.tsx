...
const PoolTable = ({
  ...
  const { data: pool } = usePool(
    !marketData && poolId != null ? { poolId } : undefined,
  );
  ...
  if (marketData?.neoPool?.account?.balances) {
    ...
    const sortedBalances = [...balances].sort((a, b) => {
      ...
      try {
        ...
        const aIndex =
          activeMarket?.outcomeAssets?.findIndex((marketAsset: any) => {
            if (
              typeof marketAsset === "string" &&
              marketAsset.includes(aAsset.combinatorialToken)
            ) {
              return true;
            }
            return JSON.stringify(marketAsset).includes(
              aAsset.combinatorialToken,
            );
          }) ?? -1;
        ...
        const bIndex =
          activeMarket?.outcomeAssets?.findIndex((marketAsset: any) => {
            if (
              typeof marketAsset === "string" &&
              marketAsset.includes(bAsset.combinatorialToken)
            ) {
              return true;
            }
            return JSON.stringify(marketAsset).includes(
              bAsset.combinatorialToken,
            );
          }) ?? -1;
