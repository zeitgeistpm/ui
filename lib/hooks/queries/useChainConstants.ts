...
export const useChainConstants = () => {
  ...
  return useQuery<ChainConstants>([id, sdk, "chain-constants"], async () => {
    ...
    const blockTimeSec = (consts.timestamp.minimumPeriod.toNumber() * 2) / 1000;
    const config: ChainConstants = {
      ...
      markets: {
        ...
        disputeFactor: consts.predictionMarkets.disputeFactor.toNumber() / ZTG,
      ...
      balances: {
        existentialDeposit: consts.balances.existentialDeposit.toNumber() / ZTG,
      },
    };
  });
};
