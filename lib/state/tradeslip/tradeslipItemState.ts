...
export const getItemSum = (
  item: TradeSlipItem,
  data?: UseTradslipRemoteDataQuery,
) => {
  ...
  if (item.action === "buy") {
    return calcInGivenOut(
      data.poolZtgBalance,
      data.ztgWeight,
      data.poolAssetBalance,
      data.assetWeight,
      amount,
      data.swapFee.div(ZTG),
    );
  } else {
...
export const useTradeSlipItemMax = (item: TradeSlipItem) => {
  const { data, isFetched } = useTradslipItemState(item);

  if (!isFetched || !data) return new Decimal(0);

  if (item.action === "buy") {
    const maxTokens = isNA(data.traderZtgBalance)
      ? new Decimal(Infinity)
      : data.traderZtgBalance.div(data.asset?.price.div(ZTG) ?? 0);
    if (data.tradeablePoolBalance?.lte(maxTokens)) {
      return data.tradeablePoolBalance;
    } else {
      return maxTokens;
    }
  } else {
    if (data.tradeablePoolBalance?.lte(data.traderAssetBalance)) {
      return data.tradeablePoolBalance;
    } else {
      return data.traderAssetBalance;
    }
  }
};
